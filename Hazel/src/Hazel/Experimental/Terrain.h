...
namespace Hazel
{
	...
	public:
		...
		uint32_t GetHumidityMapID() { return m_Humiditymap->GetRendererID(); }
		void NewSeed() { srand(time(NULL)); p_Seed = rand() % 32767; }
	...
	private:
		...
		csTexture* m_Humiditymap;
		csTexture* m_RandomTexture;
		float* random_pxd = nullptr;
		glm::vec3 m_SunPosition = glm::vec3(0.0, 10.0, 0.0);
		glm::vec2* m_Vertices;
		uint32_t* m_Indices;
		Ref<VertexArray> m_VA;
		Ref<Shader> m_RenderShader = Shader::Create("assets/shaders/TerrainShader2.glsl");
		Ref<Shader> cs_Erosion = Shader::Create("assets/shaders/compute/Erosion2.glsl");
		std::string _textureStr = "assets/textures/testhmap.png";
