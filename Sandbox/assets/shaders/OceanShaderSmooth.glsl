...
void main()
{
	...
	vec2 XNx2 = texture(X2, ix2).rb;
	vec2 ZNz0 = texture(Z0, ix0).rb;
	vec2 ZNz1 = texture(Z1, ix1).rb;
	vec2 ZNz2 = texture(Z2, ix2).rb;
	float Y0 = texture(Y0, ix0).r;
	float Y1 = texture(Y1, ix1).r;
	float Y2 = texture(Y2, ix2).r;

	// GEOMETRY
	float dx = XNx0.r + XNx1.r + XNx2.r;
	float dy = Y0 + Y1 + Y2;
	float dz = ZNz0.r + ZNz1.r + ZNz2.r;
	...
	normalAlt = Normal;
	viewVectorML = gl_Position.xyz - u_ViewPosition.xyz;
...
{	
	...
	float sx2 = texture(X2, ix2).b;
	float sz2 = texture(Z2, ix2).b;
	...
	//////////////////////////
	vec3 normal1 = Normal;//normalize(normalVec);
	vec3 light_vector1 = normalize(lightVec);
	vec3 halfway_vector1 = normalize(halfwayVec);
	vec3 view_vector1 = normalize(viewVectorML);//normalize(viewVec);

	//float d = dot(normal1, light_vector1);
	float d = -dot(normal1, view_vector1);
	bool facing = d > 0.0;
	...
	fragColor = C_Ambient +
				...
				(facing ?
					C_Emissive * max(pow(dot(normal1, view_vector1), 10.0), 0.0) :
					vec4(0.0, 0.0, 0.0, 0.0));
